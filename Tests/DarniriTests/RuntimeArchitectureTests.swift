...
final class RuntimeArchitectureTests: XCTestCase {
    func testSnappyWindowMovementBezierStartsAndEndsAtBounds() {
        let config = CubicConfig.snappyWindowMovement
        let startTime = 4.0
    ...
    func testSnappyWindowMovementBezierIsMonotonicAndSnappy() {
        let config = CubicConfig.snappyWindowMovement
        let startTime = 9.0
        let animation = CubicAnimation(
            from: 0.0,
    ...
    @MainActor
    func testNiriNativeMacOSTabRekeysInvisibleSiblingWithoutDarniriTab() async throws {
        let columnWidth: CGFloat = 320
        let controller = Self.controller()
        let workspaceId = try XCTUnwrap(controller.workspaceManager.workspaceId(for: "1", createIfMissing: true))
        _ = controller.workspaceManager.focusWorkspace(named: "1")
    ...
    func testNiriManagedReplacementProtectedRemovalResolvesFocusFallbackDuringValidation() async throws {
        ...
        let metadata = ManagedReplacementMetadata(
            bundleId: "com.darniri.tests.tabs",
            workspaceId: workspaceId,
            mode: .tiling,
    ...
    private static func nativeTabBundleId(pid: pid_t) -> String {
        "com.darniri.tests.native-tabs.\(pid)"
    }
    ...
    private static func settingsStore(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("DarniriTests-\(UUID().uuidString)", isDirectory: true)
